...
import { editKendaraan } from '@/business/kendaraanService';
...
export default function KendaraanTable({
    ...
    const handleDelete = async (kendaraan: any) => {
        if (confirm(`Apakah Anda yakin ingin ${kendaraan.aktif ? 'menonaktifkan' : 'mengaktifkan'} kendaraan dengan nopol ${kendaraan.no_polisi}?`)) {
            kendaraan.aktif = !kendaraan.aktif;
            await editKendaraan(kendaraan.no_polisi, kendaraan);
            router.refresh();
        }
    }
    ...
    return (<>
            ...
            <table className="min-w-full text-left border-collapse">
                <thead>
                    <tr className="bg-gray-50 text-gray-700 border-b border-gray-200">
                        ...
                        <th
                            onClick={() => HandleSort(searchParams, router,'manufaktur', path)}
                            className="py-3 px-4 font-semibold text-sm cursor-pointer hover:bg-gray-100 transition-colors"
                        >
                            Manufaktur {renderSortIcon('manufaktur')}
                        </th>
                        <th className="py-3 px-4 font-semibold text-sm">Jenis</th>
                        <th className="py-3 px-4 font-semibold text-sm">Tahun</th>
                        <th className="py-3 px-4 font-semibold text-sm">Warna</th>
                ...
                <tbody>
                    {data.length === 0 ? (
                        ...
                        data.map((kendaraan) => (
                            <tr key={kendaraan.no_polisi} className="border-b border-gray-200 hover:bg-gray-100">
                                <td className="py-3 px-4 text-sm">{kendaraan.no_polisi}</td>
                                <td className="py-3 px-4 text-sm">{kendaraan.nama}</td>
                                <td className="py-3 px-4 text-sm">{kendaraan.manufaktur}</td>
                                <td className="py-3 px-4 text-sm">{kendaraan.jenis}</td>
                                <td className="py-3 px-4 text-sm">{kendaraan.tahun}</td>
                                <td className="py-3 px-4 text-sm">{kendaraan.warna}</td>
                                <td className="py-3 px-4 text-sm">
                                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${kendaraan.aktif ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                        }`}>
                                        {kendaraan.aktif ? 'Aktif' : 'Non Aktif'}
                                    </span>
                                </td>
                                <td className="py-3 px-4 text-center">
                                    <div className="flex items-center justify-center gap-2">
                                        <a
                                            href={`/kendaraan/edit/${kendaraan.no_polisi}`}
                                        ...
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded shadow-sm text-xs font-medium text-white transition-colors ${kendaraan.aktif
                                                    ? 'bg-red-600 hover:bg-red-700'
                                                    : 'bg-emerald-600 hover:bg-emerald-700'
                                                    }`}
                                            >
                                                {kendaraan.aktif ? 'Hapus' : 'Aktifkan'}
                                        </button>
                                    </div>
                                </td>
                            </tr>
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div className="flex items-center gap-2 self-end sm:self-auto">
                <a
                    href={`/${path}?page=${currentPage - 1}&status=${currentStatus}&limit=${currentLimit}&search=${currentSearch}`}
